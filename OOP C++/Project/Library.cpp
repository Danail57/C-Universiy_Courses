/*Задача 1:
Да се създаде клас CDocument, съдържащ следните членове:
Номер на документ;
Име на притежател;
Дата на издаване (обект от клас CDate);
Дата на валидност (обект от клас CDate);
Конструктор без параметри и с входни аргументи;
Член-функции getData() и display().
Да се създаде производен клас CStudentCard, който съдържа:
Факултетен номер;
Специалност;
Конструктор без параметри и с входни аргумент;
Член-функции getData() и display().
Да се създаде производен клас CLibraryCard, който съдържа:
Номер на читател;
Брой заети книги;
Конструктор без параметри и с входни аргумент;
Член-функции getData() и display().
Да се създаде програма, която:
Въвежда се тип карта;
Въвежда в два масива според типа n карти (n ≤ 30);
Извежда всички карти;
Извежда картите издадени на определена дата;
Позволява търсене по номер на документ.
Задача 2:
Да се допълни клас CDate като се реализират следните оператори:
operator !=
operator <
operator >
operator <=
operator >=
operator + (добавяне на брой дни към дата)
Да се разшири задача 1 като се добавят следните функционалности:
Удължаване срока на валидност на карта с определен брой дни;
Извеждане на картите с изтекъл срок;
Извеждане на валидните карти към текущата дата;
Извеждане на картите, чийто срок изтича до определен брой дни.*/
