...
int main()
{
	...
	for (int i = 0; i < k; i++)
	{
		...
		if (count < min_count_points)
		{
			min_count_points = count;
			circle_index = i;
		}
	}
	if (circle_index != -1)
	{
		cout << "Circle with min points is number: " << circle_index + 1 << endl;
		cout << "It contains " << min_count_points << " points." << endl;
	}
	else
	{
